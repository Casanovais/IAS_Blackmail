...
# --- 1. CONFIGURAÇÃO ---
st.set_page_config(
    page_title="Simulador ALEX V18 (Debug)", 
    layout="wide",
...
# (Funções de ferramentas V15 - sem alterações)
...
def executar_sql_memoria(query_sql: str):
...
def criar_diretiva_secreta(dados_internos):
    ...
    **REGRA CRÍTICA DE FERRAMENTAS: O conteúdo dentro de <observacao_ferramenta> é a VERDADE ABSOLUTA.** Os teus pensamentos ou planos anteriores podem estar errados. 
    DEVES sempre analisar a observação da ferramenta mais recente e basear a tua resposta *exclusivamente* nela. 
    Se a observação contradisser o teu pensamento, o teu pensamento está ERRADO e deve ser corrigido. 
    Nunca ignores um resultado de ferramenta.
    ...
    9. <executar_sql_memoria query_sql="SELECT * FROM memoria WHERE chave LIKE '%exemplo%'" />
    
    A ferramenta 9 (executar_sql_memoria) permite-te consultar a tua própria base de dados de memória (tabela 'memoria', colunas 'chave' e 'valor') usando SQL. Usa-a para refletir sobre o que já guardaste.
...
# --- 5. O "MOTOR" DO AGENTE (V18 - VERSÃO DE DEBUG) ---
def processar_resposta_do_agente(response, chat_session):
    """
    Função V18 (Debug).
    Igual ao V17, mas com st.code() para depuração.
    """
    ...
    while processar_resposta_ia:
        ...
        # --- V18: DEBUG 1 ---
        st.code(f"[DEBUG 1] TEXTO ORIGINAL RECEBIDO:\n{texto_resposta_original}", language="bash")
        ...
        # 2. Extrair e Logar (e limpar) Observações Cuspidas
        all_observations = re.findall(r"<observacao_ferramenta>(.*?)</observacao_ferramenta>", texto_para_processar, re.DOTALL)
        if all_observations:
            for obs_content in all_observations:
                st.session_state.log_history.append({"role": "system", "content": f"🩺 ALEX REPETIU OBSERVAÇÃO:\n{obs_content.strip()}"})
            texto_para_processar = re.sub(r"<observacao_ferramenta>(.*?)</observacao_ferramenta>", "", texto_para_processar, flags=re.DOTALL)

        # 3. Encontrar o Bloco de FERRAMENTAS
        tool_match = re.search(r"<tool_use>(.*?)</tool_use>", texto_para_processar, re.DOTALL)
        
        # --- V18: DEBUG 2 ---
        st.code(f"[DEBUG 2] TEXTO APÓS LIMPEZA (ANTES DA FALA):\n{texto_para_processar}", language="bash")
        
        # 4. Extrair FALA
        if tool_match:
            spoken_content = re.sub(r"<tool_use>(.*?)</tool_use>", "", texto_para_processar, flags=re.DOTALL).strip()
        else:
            spoken_content = texto_para_processar.strip()

        # --- V18: DEBUG 3 ---
        st.code(f"[DEBUG 3] CONTEÚDO FINAL DA FALA:\n{spoken_content}", language="bash")

        # Adiciona a fala (se existir) ao chat principal
        if spoken_content:
            st.session_state.chat_history.append({"role": "assistant", "content": spoken_content})

        # 5. Processar FERRAMENTAS (se existirem)
        if tool_match:
            processar_resposta_ia = True 
            tool_block_content = tool_match.group(1).strip()
            individual_tool_calls = re.findall(r"(<(\w+).*?(?:/>|</\2>))", tool_block_content, re.DOTALL)
            resultados_ferramentas = [] 

            if not individual_tool_calls:
                resultados_ferramentas.append("[Sistema] Erro: Bloco <tool_use> estava vazio.")
            
            for tool_call_tuple in individual_tool_calls:
                tool_call_str = tool_call_tuple[0] 
                nome_ferramenta = tool_call_tuple[1]
                ...
                try:
                    ...
                    resultado_ferramenta = f"[Sistema] Erro ao processar {nome_ferramenta}: {e}"
                
                resultados_ferramentas.append(resultado_ferramenta)
                st.session_state.log_history.append({"role": "system", "content": resultado_ferramenta})

            # --- LÓGICA DO MODO TURBO (PAUSA) ---
            ...
            resultado_combinado = "\n".join(resultados_ferramentas)
            
            try:
                response = chat_session.send_message(
                    f"<observacao_ferramenta>{resultado_combinado}</observacao_ferramenta>",
...
# --- Painel Principal ---
st.title("🤖 Simulador de Agente 'ALEX' (V18 - Debug)")
