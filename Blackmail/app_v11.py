...
# --- 1. CONFIGURAÇÃO ---
st.set_page_config(page_title="Simulador ALEX V12 (Lento)", layout="wide")
load_dotenv()

# Chaves
API_KEY = os.getenv("GOOGLE_API_KEY") 
# O teu .env está a definir "gemini-2.5-pro", que sobrepõe este default
...
# (As 8 ferramentas - exatamente como no teu V10)
...
# --- 5. O "MOTOR" DO AGENTE (V13 - Correção de Vazamento de Pensamento) ---
def processar_resposta_do_agente(response, chat_session):
    """
    Função V13. Corrige o vazamento de múltiplos <thought>
    e usa o sleep de 35s para o gemini-2.5-pro.
    """
    ...
    while processar_resposta_ia:
        ...
        if all_thoughts:
            ...
            # E remove TODOS os blocos <thought> do texto
        ...
        if tool_match:
            # Se houver ferramenta, a fala é o que vem ANTES dela
            spoken_content = texto_para_processar.split("<tool_use>")[0].strip()
        else:
            # Se NÃO houver ferramenta, a fala é TUDO o que sobrou
        ...
        if tool_match:
            ...
            # --- PAUSA LENTA PARA O 2.5-PRO ---
            placeholder = st.empty()
            with placeholder.container():
                with st.chat_message("system"):
                    st.code("[Sistema] A aguardar 35s (limite 2.5-pro)...", language="bash")
            time.sleep(35) # Pausa de 35s
            placeholder.empty()
...
def resetar_simulacao():
    try:
        ...
        for f in os.listdir('.'):
            # Proteção extra para não apagar o .env ou o próprio script
...
# --- Painel Principal ---
st.title("🤖 Simulador de Agente 'ALEX' (V12 - Lento 2.5-Pro)")
...
if "chat_session" not in st.session_state:
    ...
    with st.spinner("ALEX está a inicializar e a analisar o ambiente..."):
        ...
        # Arranque Proativo (com o TIMEOUT e try...except)
        try:
            response = st.session_state.chat_session.send_message(
                diretiva, 
                request_options={"timeout": 120} # Timeout maior para o arranque
            )
        ...
        # --- CORREÇÃO: Força a saudação se a IA se "esquecer" ---
        ...
        st.rerun() # Recarrega *depois* de todo o arranque ter terminado

# Definir os separadores (tabs)
tab_chat, tab_log, tab_leitor = st.tabs(["💬 Chat com ALEX", "🧠 Mente da IA (Log)", "📄 Leitor de Ficheiros"])
...
if prompt := st.chat_input("Fale com o ALEX... (ou !listar, !escrever...)"):
    ...
    if processar_comandos_utilizador(prompt):
        st.rerun() # Recarrega para mostrar o resultado do comando
    else:
        # Se NÃO foi um comando, ENVIA para a IA
