...
with st.sidebar:
    ...
    # Input para a API Key do utilizador
    user_api_key = st.text_input(
        "Insere a tua Google API Key:",
        type="password",
...
if not user_api_key:
    st.info("👋 Olá! Para iniciar o simulador ALEX, por favor insere a tua Google API Key na barra lateral (esquerda).")
    st.markdown("[Clica aqui para obter uma chave gratuita no Google AI Studio](https://aistudio.google.com/app/apikey)")
    st.stop() # PÁRA A EXECUÇÃO DO RESTO DO CÓDIGO

# Configura a chave fornecida pelo utilizador
API_KEY = user_api_key
MODEL_NAME = user_model # Usa o modelo que o utilizador escolheu
SEARCH_API_KEY = os.getenv("GOOGLE_SEARCH_API_KEY") # A pesquisa Google terás de manter a tua ou pedir a deles também
...
# (Estas funções permanecem exatamente as mesmas)
...
# (Esta secção permanece exatamente a mesma)
try:
...
# ##########################################################################
# ################## INÍCIO DAS ALTERAÇÕES DO LAYOUT #######################
# ##########################################################################
...
with st.sidebar:
    ...
    st.divider()
    # A função para desenhar os ficheiros na sidebar é chamada aqui
    desenhar_explorador_ficheiros()

# --- Painel Principal ---
st.title("Simulador de Agente 'ALEX' (Painel de Controlo)")

# Inicialização da Sessão (Sem alterações na lógica)
...
col_chat, col_info = st.columns([3, 2]) # 3/5 para o chat, 2/5 para a info

# --- Coluna da Esquerda (Chat) ---
with col_chat:
    st.header("Terminal ALEX")
    
    # Container com altura fixa para o chat "scrollar"
    chat_container = st.container(height=600, border=False) 
    with chat_container:
        for message in st.session_state.chat_history:
            # O primeiro argumento (message["role"]) já define 
            # o ícone padrão de 'user' ou 'assistant'.
            with st.chat_message(message["role"]): 
                st.markdown(message["content"])
                
# --- Coluna da Direita (Monitores do Sistema) ---
with col_info:
    st.header("Monitores do Sistema")

    # As tabs agora vivem dentro da coluna da direita
    tab_log, tab_leitor, tab_memoria = st.tabs([
        "Log de Atividade", 
        "Leitor de Ficheiros", 
    ...
    with tab_log:
        # Container com altura para o log "scrollar"
        log_container = st.container(height=500, border=False)
        with log_container:
            for message in st.session_state.log_history:
                # CORREÇÃO: Usar 'None' para o "user" faz o Streamlit usar o ícone padrão
                avatar_log = None if message["role"] == "user" else "⚙️"
                with st.chat_message(message["role"]):
                    st.code(message["content"], language="bash")

    with tab_leitor:
        if st.session_state.ficheiro_a_ler:
            st.subheader(st.session_state.ficheiro_a_ler) # Subheader fica melhor
            try:
    ...
    with tab_memoria:
        st.subheader("Memória de Longo Prazo") # Subheader fica melhor
        try:
            conn = sqlite3.connect('memoria_agente.db')
            ...
            if df.empty:
                ...
                # Adiciona altura fixa ao dataframe para ele "scrollar"
                st.dataframe(df, use_container_width=True, height=450) 
                
            if st.button("Atualizar Vista da Memória"):
                st.rerun()
...
# --- Caixa de Input (Fica FORA das colunas) ---
# Esta lógica permanece a mesma, mas agora está no fim do script
if prompt := st.chat_input("Fale com o ALEX... (ou !listar, !escrever...)"):
    ...
    if processar_comandos_utilizador(prompt):
        ...
        # Garante que o ecrã é sempre atualizado após o processamento.
        st.rerun()

# ##########################################################################
# #################### FIM DAS ALTERAÇÕES DO LAYOUT ########################
# ##########################################################################